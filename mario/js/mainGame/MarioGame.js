...
function MarioGame() {
  ...
  this.init = function(levelMaps, level) {
    ...
    if (!mario) {
      //so that when level changes, it uses the same instance
      mario = new Mario();
      // Week 4 Assignment, Task 1. In the line below, call the init() on the instantiated mario object above so that if mario is not initialized (!mario), mario will become initialized.
      mario.init();
    } else {
      // Week 4 Assignment, Task 2a. In the line below, set the mario object attribute x to 10 in the line below so that mario starts at the far left of the screen.
      mario.x = 10;
      // Week 4 Assignment, Task 2b. In the line below, set the mario frame object to 0 below so that the mario sprite is reset to the initial frame.
      mario.frame = 0      
    }
    element = new Element();
  ...
  this.renderMap = function() {
    ...
    // Week 4 Assignment, Task 3. In the line below, set the grounded attribute of the mario object to false so that mario falls off the platform and not hover around
  ...
  this.checkElementMarioCollision = function(element, row, column) {
    ...
    if (collisionDirection == 'l' || collisionDirection == 'r') {
      ...
      if (element.type == 3) {
        //PowerUp Box
        // Week 4 Assignment, Task 4. In the line below, create a new instance of a PowerUp object and assign that instance to a variable named powerUp.
        ...
        // Week 4 Assignment, Task 5. In the line below, call the play method on the gameSound object and pass 'powerupAppear' to the method in the first parameter.
